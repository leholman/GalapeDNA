...
# Example data frame of pairwise site IDs (Start x End)
sitePairwiseDist <- expand.grid(
  Start = metadatSites$SiteID,
  End   = metadatSites$SiteID
)

for (row in 1:dim(sitePairwiseDist)[1]){

LoopFrom <- sitePairwiseDist$Start[row]
LoopTo <- sitePairwiseDist$End[row]


loopPath <- shortestPath(tr_depth_corrected, c(places2[LoopFrom,1],
                                               places2[LoopFrom,2]),
                                               c(places2[LoopTo,1],
                                                 places2[LoopTo,2]), 
                                                 output="SpatialLines")
sitePairwiseDist$dist[row] <- SpatialLinesLengths(loopPath, longlat = TRUE)
...
write.csv(sitePairwiseDist.d.out,"distanceData/workingFolderJan25/SiteDistanceMatrix.csv")
write.csv(sitePairwiseDist,"distanceData/workingFolderJan25/SiteDistance.csv")
write.csv(pathPointsTable,"distanceData/workingFolderJan25/pathPointsTable.csv")
